...
pub fn validate_function_name(function_name: &str) -> Result<()> {
    ...
    if !function_name.chars().next().unwrap().is_ascii_alphabetic() && function_name.chars().next().unwrap() != '_' {
        return Err(anyhow!(
    ...
    if !function_name.chars().all(|c| c.is_ascii_alphanumeric() || c == '_') {
        return Err(anyhow!(
...
pub fn validate_hex_value(value_str: &str) -> Result<alloy::primitives::U256> {
    ...
    let value = if value_str.starts_with("0x") || value_str.starts_with("0X") {
        ...
        alloy::primitives::U256::from_str(value_str)
            .map_err(|_| anyhow!("Invalid numeric value: '{}'. Use decimal format or '0x' prefixed hex", value_str))?
    };
...
pub fn interpret_rpc_error(error: &str) -> String {
    if error.contains("execution reverted") {
        ...
        "Transaction failed: Gas limit too low. Try increasing the gas limit for this transaction.".to_string()
        ...
        "Network error: Request timed out. The RPC endpoint may be overloaded or unreachable.".to_string()
...
pub fn interpret_abi_error(error: &str, contract_address: &str) -> String {
    if error.contains("404") || error.contains("not found") {
        ...
        "Network error: Cannot connect to Etherscan API. Check your internet connection.".to_string()
    } else if error.contains("timeout") {
...
mod tests {
    ...
    fn test_validate_address() {
        ...
        assert!(validate_address("0xgg2d35Cc6435C9c1c72c5E7b18BaB7e1DB7a5d6e").is_err()); // Invalid hex
    }
